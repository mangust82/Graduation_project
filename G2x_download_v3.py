"""the module downloads the configuration file to the g2x analyzer using the 
...
g2x_username = '******'
g2x_password = '*****'
IP_address = '10.XX.XXX.XX'
conf_name = str(Path("C:\work\IQ\IQAlias_G2X-MSK-Test2-B2.xls"))
...
def load_conf_G2X(g2x_username:str, g2x_password:str, IP_address:str, conf_name:str) -> None:
    """Function loads configuration *.csv file on the IPTV analuser G2X via Selenium"""
    ...
    try:
        ...
        dowload_conf = driver.find_element(By.XPATH, '/html/body/div[1]/div/div[5]/a[1]') 
        dowload_conf.click() 
        time.sleep(1) 
...
if __name__ == '__main__':
    start_time = time.time()
    load_conf_G2X(g2x_username, g2x_password, IP_address, conf_name)
    end_time = time.time()
